...
router.put('/editar/:id', authUser,conectarBancoDados, async function(req, res) {
  try{
    ...
    const tarefaAtualizada = await EsquemaTarefa.updateOne({_id: idTarefa}, {posicao, titulo, descricao, status, dataEntrega, usuarioCriador});
...
router.delete('/deletar/:id', authUser,conectarBancoDados, async function(req, res) {
  try{
    ...
    const respostaBD = await EsquemaTarefa.deleteOne({_id: idTarefa}).populate('usuarioCriador');
    res.status(200).json({
      status: "OK",
